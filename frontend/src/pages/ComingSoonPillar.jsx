...
export default function ComingSoonPillar() {
  ...
  const { t } = useLanguage();
  const copy = t('comingSoon');
  const pillarCopy = pillar ? t('lessonsHub').pillars[pillar.key] ?? pillar : null;
  ...
  if (!pillar) {
    return (
      <div className="container" style={{ padding: 60, textAlign: 'center' }}>
        <h1 className="page-title">{copy.trackNotFound}</h1>
        <Link to="/lessons" className="btn btn-primary">{copy.backToHub}</Link>
      </div>
    );
  }
  ...
  return (
    <div className="container" style={{ padding: '48px 0', maxWidth: 640, textAlign: 'center' }}>
      ...
      <span className="badge badge-locked">{copy.comingSoonBadge}</span>
      <h1 className="page-title" style={{ marginTop: 12 }}>
        {pillar.icon} {pillarCopy.name}
      </h1>
      <p className="page-subtitle">{pillarCopy.tagline}</p>

      <div className="card" style={{ textAlign: 'left', marginTop: 24 }}>
        <p style={{ margin: '0 0 12px', fontWeight: 700, color: 'var(--color-blue)' }}>{copy.whatsPlanned}</p>
        <ul style={{ margin: 0, paddingLeft: 20, color: '#4b5a6a', lineHeight: 1.8 }}>
          {(pillarCopy.items ?? pillar.items).map((item) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
      </div>
      ...
      <div className="card" style={{ marginTop: 20 }}>
        {notifySubmitted ? (
          <p style={{ margin: 0, color: 'var(--color-green)', fontWeight: 700, textAlign: 'center' }}>
            {copy.thanksNotify.replace('{name}', pillarCopy.name)}
          </p>
        ) : (
          <form onSubmit={handleNotify} style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button type="submit" className="btn btn-primary" disabled={notifying}>
              {notifying ? copy.saving : copy.notifyMe}
            </button>
          </form>
        )}
